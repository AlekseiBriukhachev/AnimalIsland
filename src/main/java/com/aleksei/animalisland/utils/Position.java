...
import com.aleksei.animalisland.utils.enums.Direction;
...
public class Position {
    ...
    public Position moveDirectTo(Direction direction) {
        return moveDirectTo(direction, 1);
    }

    public Position moveDirectTo(Direction direction, int steps) {
        return new Position(
    ...
    public Set<Position> lookAround(int radius, Sized sized) {
        ...
        for (int ry = y - radius; ry <= y + radius; ry++) {
            for (int rx = x - radius; rx <= x + radius; rx++) {
                Position candidate = Position.getPosition(rx, ry);
